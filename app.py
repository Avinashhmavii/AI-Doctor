...
# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %( fête

System: It looks like the response was cut off, possibly due to a length limitation or an error during generation. Based on your request, you encountered an error (`Error analyzing image and query: 'choices'`) in the output, indicating an issue with the OpenRouter API response parsing in the `analyze_image_and_voice` function. The goal is to update the previous `app.py` to improve error handling, log the raw API response for debugging, and ensure compatibility with the OpenRouter API response structure, while retaining the same `artifact_id` as the previous artifact.

Below is the complete, updated `app.py` with the following changes:
1. Enhanced error handling in the `analyze_image_and_voice` and `generate_ai_response` functions to catch and log issues with the API response.
2. Logging of the raw API response to help diagnose issues with the response structure.
3. A check for the `"choices"` key and fallback behavior if it’s missing or the response is malformed.
4. Preservation of all other functionality from the previous version, with only the necessary changes to address the error.

Since this is an update to the previous artifact, I’m using the same `artifact_id` (`47b5ae76-5e3a-4768-ab23-a8286ce3a03b`) as specified.

<xaiArtifact artifact_id="47b5ae76-5e3a-4768-ab23-a8286ce3a03b" artifact_version_id="0889cc92-7dc3-400e-ad0d-e60c0c916353" title="app.py" contentType="text/python">
from flask import Flask, render_template, request, jsonify
import os
from dotenv import load_dotenv
import base64
...
"""
        ...
        if "choices" not in response_json or not response_json["choices"]:
            logger.error("No 'choices' key in API response or choices is empty")
            return "Error: Invalid response from OpenRouter API (no choices found)"
        ...
        logger.error(f"Key error in parsing OpenRouter API response: {str(e)}")
        return f"Error: Invalid response structure from OpenRouter API - {str(e)}"
    except Exception as e:
    ...
    try:
        ...
        if 'audio' in request.files and request.files['audio'].filename != '':
            ...
            if transcribed_text:
                ...
                if state["encoded Leurs
System: It appears that the response was cut off again, likely due to a length limitation or an error during generation. I’ll provide the complete, updated `app.py` file, ensuring all changes are included to address the error (`Error analyzing image and query: 'choices'`) you encountered. The changes include enhanced error handling, logging of the raw OpenRouter API response, and a check for the `"choices"` key in the API response, while preserving the same `artifact_id` (`47b5ae76-5e3a-4768-ab23-a8286ce3a03b`) as the previous artifact.

This version completes the code, including the full `analyze` route and all other functions, with no truncation. The key updates are:
1. Improved error handling in `analyze_image_and_voice` and `generate_ai_response` to catch network errors, key errors, and unexpected exceptions.
2. Logging of the raw API response for debugging.
3. Validation of the `"choices"` key in the API response, with a fallback error message if it’s missing.

Here’s the complete updated code:

<xaiArtifact artifact_id="47b5ae76-5e3a-4768-ab23-a8286ce3a03b" artifact_version_id="75945a72-9195-44de-a4b9-17d9cf9d3c6b" title="app.py" contentType="text/python">
from flask import Flask, render_template, request, jsonify
import os
from dotenv import load_dotenv
import base64
